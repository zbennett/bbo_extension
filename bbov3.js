...
var app = { "startTime": Date.now(), "traffic": '', "trafficCounter": 0, "prevWStime": 0,
		...
		"pendingDD" : [], "play": {}, "alert": {}, "prefLoaded": false,
	...
	if ( logme ) {
		...
		if (pref.appTrafficLoggingFull || 
			...
			const phphtml = wasPHP ? ' <span class="xhr-post">' + 
					url.substr(url.lastIndexOf('/')+1) + '</span> ' : '';
...
async function localePhrases() {
	...
	if (!response.ok) {
		console.warn('Attempt to fetch %s failed with HTTP status code %d', 
			url, response.status);
		return;
	}
	...
	console.info('Fetched %s-locale phrases; seats: %s %s %s %s, honors: %s, ' +
...
function trafficPrefix(timeStamp) {
	...
	let localHHMMSSmsec = localHHMMSS.substr(0,localHHMMSS.length-3) + '.' + 
		zeroPadInt(edate.getMilliseconds(), 3) + ' ' + localHHMMSS.substr(-2);
	
	let relTime = (timeStamp + performance.timeOrigin - app.startTime) / 1000;
	let html = `<p id="p${app.trafficCounter}">` + zeroPadInt(app.trafficCounter, 4) +
...
function processXHR(d) {
	...
	let url = d.url;
	if (d.method !== 'POST' || d.responseType !== 'text' || !url.endsWith('.php')) { 
		return; 
	}
	if ( !d.response.startsWith('<?xml') ) { return; }   // Guard
...
function processWebsocket(e) {
	...
	// all begin with <cs
	let mtype = msg.startsWith('<') ? msg.substr(1, msg.search(' ')-1) : 
			...
			if (app.deal !== undefined) {
				if ( !app.deal.timingSaved ) {
					...
					console.warn('BBO Helper: Saving board timing for previous deal ' + 
						...
						app.table.style, app.table.type, app.table.tkey, app.table.title);
					saveDealTiming();
				}	
				app.deal = undefined;
			}
...
async function rightPaneWatcher() {
	...
	if (rightDiv === null) { return; }  // Guard against changes.
	...
	async function WOSfinder(el) {
		...
		setTimeout( () => { obsWOS.disconnect(); }, 5000 );  // Try for 5 sec
	}
	...
	function screenChildElement(mutations) {
		...
		for (let mr of mutations) {
			for (let el of mr.addedNodes) {
				...
				if (el.tagName === 'WHOS-ONLINE-SCREEN') { wos = el; break; }
			}
		}
...
function trickcard(card, timeStamp, isMe) {
	...
	// are receiving catch-up messages. Assign these a TDIFF of 0 because we
	// have no timing information.
	let tdiff = app.deal.blast1_complete ? timeStamp - app.deal.lastActionTime : 0;
...
function mh_hand(response) {
	...
	if (au !== null) {  // NULL if no calls were made
		let lix = au.length - 1;
		let completedAuction = au.length > 3 && au[lix] === 'p' &&
...
async function showDoubleDummy(d, dd) {
	...
	if (d.bnum !== bnum) {
		...
		if (bnum === 99) { console.log('Board 99'); return; } /// 
...
async function getDealViaDOM(mode, needAuction) {
	...
	for (let itry=0; itry<maxTries; itry++) {
		...
		if ( tpc[0].style.display === 'none' ) {
			...
			let denomLetter = (symbol === '♠') ? 'S' : (symbol === '♥') ? 'H' : 
				(symbol === '♦') ? 'D' : (symbol === '♣') ? 'C' : 'N';
...
async function toggleNameDisplay() {
	...
	for (let i=0; i<4; i++) {
		...
		if (p1) {
			desc[i] = p1.fullname;
			if (p1.state !== '') { desc[i] += ' (' + p1.state + ')'; }
		}
		else if (p2 && p2.name !== '') {
			desc[i] = p2.name;
			if (p2.country !== '') { desc[i] += ' (' + p2.country + ')';  }
		}
		else {
...
function auctionContract() {
	...
	for (i1=au.length-4; i1>= 0; i1--) { if (au[i1].length === 2) { break; } }
...
function savetraffic() {
	...
	let startStrHTML = sdate.toDateString() + ' ' + strLocalTime.substr(0,8) + 
		' - ' + endHHMMSS;		
	let fname = 'BBO Traffic ' + startStrFname + ' - ' + 
		endHHMMSS.substr(0,8).replace(/:/g, '.') + '.htm';
...
`;
	...
	if (!pref.appTrafficLoggingFull) {
		let msg = 'This log does not include cs_ping messages, <sc_ack> responses, ' + 
